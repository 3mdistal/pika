...
describe('copyFixtureTemplates', () => {
  it('should copy all templates when types is undefined', async () => {
    ...
    try {
      ...
      const ideaDefault = await vaultFileExists(vaultPath, '.bwrb/templates/idea/default.md');
      expect(ideaDefault).toBe(true);
      
      // Check that task templates exist
      const taskDefault = await vaultFileExists(vaultPath, '.bwrb/templates/task/default.md');
      expect(taskDefault).toBe(true);
      
      const taskBugReport = await vaultFileExists(vaultPath, '.bwrb/templates/task/bug-report.md');
  ...
  it('should handle non-existent types gracefully', async () => {
    ...
    try {
      ...
      const templatesDir = path.join(vaultPath, '.bwrb', 'templates');
  ...
  it('should preserve template content correctly', async () => {
    ...
    try {
      ...
      const content = await readVaultFile(vaultPath, '.bwrb/templates/idea/default.md');
